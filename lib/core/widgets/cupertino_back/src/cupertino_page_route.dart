...
mixin CupertinoRouteTransitionMixin<T> on PageRoute<T> {
  ...
  static bool _isPopGestureEnabled<T>(PageRoute<T> route) {
    ...
    // this route will reject a pop, so bail out
...
class _PageBasedCupertinoPageRoute<T> extends PageRoute<T> with CupertinoRouteTransitionMixin<T> {
  _PageBasedCupertinoPageRoute({required CupertinoPage<T> page}) : super(settings: page) {
...
class _CupertinoBackGestureDetectorState<T> extends State<_CupertinoBackGestureDetector<T>> {
  ...
  void _handleDragStart(DragStartDetails details) {
    assert(mounted, 'Tried to start a gesture on a disposed CupertinoRouteBackGestureDetector');
    assert(_backGestureController == null, 'Started a gesture before ending the previous one.');
    _backGestureController = widget.onStartPopGesture();
  }

  void _handleDragUpdate(DragUpdateDetails details) {
    assert(mounted, 'Tried to update a gesture on a disposed CupertinoRouteBackGestureDetector');
  ...
  void _handleDragCancel() {
    assert(mounted, 'Tried to cancel a gesture on a disposed CupertinoRouteBackGestureDetector');
