...
import '../bloc/expense_requests_bloc.dart';
import '../widgets/expense_request_card.dart';

/// Xarajat so'rovlari ro'yxati ekrani (`GET /expense-request/`) — Moliya
/// tabidagi "Xarajat so'rovlari" kartasidan push qilinadi (orqaga tugmasi
/// bor). Figma: node 1881-316875 (light) / 1881-324777 (dark).
class ExpenseRequestsListPage extends StatelessWidget {
  const ExpenseRequestsListPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider<ExpenseRequestsBloc>(
      create: (_) =>
          getIt<ExpenseRequestsBloc>()..add(const ExpenseRequestsRequested()),
      child: const _ExpenseRequestsView(),
    );
  }
}

class _ExpenseRequestsView extends StatefulWidget {
  const _ExpenseRequestsView();

  @override
  State<_ExpenseRequestsView> createState() => _ExpenseRequestsViewState();
}

class _ExpenseRequestsViewState extends State<_ExpenseRequestsView> {
  ...
  void _onScroll() {
    ...
    if (position.pixels >= position.maxScrollExtent - 300) {
      context.read<ExpenseRequestsBloc>().add(const ExpenseRequestsLoadMore());
    }
  }

  Future<void> _onRefresh(BuildContext context) {
    final bloc = context.read<ExpenseRequestsBloc>();
    bloc.add(const ExpenseRequestsRequested());
    return bloc.stream.firstWhere(
      (s) => s.status != ExpenseRequestsStatus.loading,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            const _ExpenseRequestsHeader(),
            Expanded(
              child: RefreshIndicator(
                color: colors.accentSub,
                onRefresh: () => _onRefresh(context),
                child: BlocBuilder<ExpenseRequestsBloc, ExpenseRequestsState>(
                  builder: (context, state) {
                    switch (state.status) {
                      case ExpenseRequestsStatus.loading:
                      case ExpenseRequestsStatus.initial:
                        return const _CenteredScrollable(
                          child: CircularProgressIndicator(),
                        );
                      case ExpenseRequestsStatus.failure:
...
/// Sarlavha qatori: orqaga + "Xarajat so'rovlari" + qidiruv + filtr. Qidiruv
/// ikonkasi bosilganda butun qator to'liq kenglikdagi qidiruv maydoniga
/// almashadi.
class _ExpenseRequestsHeader extends StatefulWidget {
  const _ExpenseRequestsHeader();

  @override
  State<_ExpenseRequestsHeader> createState() => _ExpenseRequestsHeaderState();
}

class _ExpenseRequestsHeaderState extends State<_ExpenseRequestsHeader> {
  ...
  void _closeSearch() {
    ...
    if (hadText) {
      context.read<ExpenseRequestsBloc>().add(
        const ExpenseRequestsSearchChanged(''),
      );
    }
  }
  ...
  void _onChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () {
      if (mounted) {
        context.read<ExpenseRequestsBloc>().add(
          ExpenseRequestsSearchChanged(value.trim()),
        );
      }
    });
  }
  ...
  Future<void> _openFilter() async {
    final bloc = context.read<ExpenseRequestsBloc>();
    final result = await context.pushNamed<Object?>(
      Routes.expenseRequestsFilter.name,
      extra: bloc.state.filter,
    );
    if (result is ExpenseRequestFilter) {
      bloc.add(ExpenseRequestsFilterChanged(result));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        ...
        SizedBox(width: 8.w),
        Expanded(
          child: l10n.financeExpenseRequests
              .s(17.sp)
              .w(800)
              .c(colors.textStrong)
              .a(TextAlign.center)
        ...
        SizedBox(width: 12.w),
        BlocBuilder<ExpenseRequestsBloc, ExpenseRequestsState>(
          buildWhen: (p, c) =>
