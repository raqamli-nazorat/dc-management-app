...
/// Moliya tarixi ro'yxati ekrani (`GET /ledger/`) — Moliya tabidagi "Tarix"
...
class LedgerListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<LedgerBloc>(
      create: (_) => getIt<LedgerBloc>()..add(const LedgerRequested()),
...
class _LedgerViewState extends State<_LedgerView> {
  ...
  void _onScroll() {
    ...
    if (position.pixels >= position.maxScrollExtent - 300) {
      context.read<LedgerBloc>().add(const LedgerLoadMore());
    }
  }

  Future<void> _onRefresh(BuildContext context) {
    final bloc = context.read<LedgerBloc>();
    bloc.add(const LedgerRequested());
    return bloc.stream.firstWhere((s) => s.status != LedgerStatus.loading);
  }
                        ...
                        if (state.items.isEmpty) {
                          return _CenteredScrollable(
                            child: AppLocalizations.of(
                              context,
                            ).ledgerEmpty.s(14.sp).w(500).c(colors.textSub),
                          );
                        }
...
/// Sarlavha qatori: orqaga + "Tarix" + qidiruv + filtr. Qidiruv ikonkasi
/// bosilganda butun qator to'liq kenglikdagi qidiruv maydoniga almashadi.
class _LedgerHeader extends StatefulWidget {
  const _LedgerHeader();

  @override
  State<_LedgerHeader> createState() => _LedgerHeaderState();
}

class _LedgerHeaderState extends State<_LedgerHeader> {
  ...
  void _closeSearch() {
    ...
    if (hadText) {
      context.read<LedgerBloc>().add(const LedgerSearchChanged(''));
    }
  }
  ...
  void _onChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () {
      if (mounted) {
        context.read<LedgerBloc>().add(LedgerSearchChanged(value.trim()));
      }
    });
  }
  ...
  Future<void> _openFilter() async {
    ...
    final result = await context.pushNamed<Object?>(
      Routes.ledgerFilter.name,
      extra: bloc.state.filter,
    );
    if (result is LedgerFilter) {
      bloc.add(LedgerFilterChanged(result));
    }
  }
