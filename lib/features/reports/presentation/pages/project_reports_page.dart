...
import '../bloc/project_reports_bloc.dart';
import '../widgets/project_report_card.dart';

/// Loyihalar bo'yicha hisobot ro'yxati ekrani (`GET /reports/projects/`).
class ProjectReportsPage extends StatelessWidget {
  const ProjectReportsPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider<ProjectReportsBloc>(
      create: (_) =>
          getIt<ProjectReportsBloc>()..add(const ProjectReportsRequested()),
      child: const _ProjectReportsView(),
    );
  }
}

class _ProjectReportsView extends StatefulWidget {
  const _ProjectReportsView();

  @override
  State<_ProjectReportsView> createState() => _ProjectReportsViewState();
}

class _ProjectReportsViewState extends State<_ProjectReportsView> {
  ...
  void _onScroll() {
    ...
    if (position.pixels >= position.maxScrollExtent - 300) {
      context.read<ProjectReportsBloc>().add(const ProjectReportsLoadMore());
    }
  }

  Future<void> _onRefresh(BuildContext context) {
    final bloc = context.read<ProjectReportsBloc>();
    bloc.add(const ProjectReportsRequested());
    return bloc.stream.firstWhere(
      (s) => s.status != ProjectReportsStatus.loading,
    );
  }
          ...
          children: [
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                child: BlocBuilder<ProjectReportsBloc, ProjectReportsState>(
                  builder: (context, state) {
                    switch (state.status) {
                      case ProjectReportsStatus.loading:
                      case ProjectReportsStatus.initial:
                        return const _CenteredScrollable(
                          child: CircularProgressIndicator(),
                        );
                      case ProjectReportsStatus.failure:
                      ...
                      case ProjectReportsStatus.success:
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return ProjectReportCard(report: state.items[i]);
                          },
                        );
                    }
                  },
...
/// Sarlavha qatori: orqaga + "Loyiha bo'yicha" + qidiruv + filtr (filtr —
/// stub, hozircha sahifasi yo'q). Qidiruv ikonkasi bosilganda butun qator
/// to'liq kenglikdagi qidiruv maydoniga almashadi (xodim bo'yicha hisobot
/// sahifasidagi naqsh bilan bir xil).
...
class _ReportsHeaderState extends State<_ReportsHeader> {
  ...
  void _closeSearch() {
    ...
    if (hadText) {
      context.read<ProjectReportsBloc>().add(
        const ProjectReportsSearchChanged(''),
      );
    }
  }
  ...
  void _onChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () {
      if (mounted) {
        context.read<ProjectReportsBloc>().add(
          ProjectReportsSearchChanged(value.trim()),
        );
      }
    });
  }
  ...
  Future<void> _openFilter() async {
    ...
    final result = await context.pushNamed<Object?>(
      Routes.projectReportsFilter.name,
      extra: bloc.state.filter,
    );
    if (result is ProjectReportFilter) {
      bloc.add(ProjectReportsFilterChanged(result));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        ...
        SizedBox(width: 12.w),
        BlocBuilder<ProjectReportsBloc, ProjectReportsState>(
          buildWhen: (p, c) =>
