...
### Projects: list, create/edit, and detail
`features/projects/` follows the three-layer split. `AddProjectPage` is the **single** form for create, edit, and read-only detail (`project: Project?`, `readOnly`, `screenTitle`) — don't fork a separate edit widget; `EditProjectPage`/`ProjectDetailsPage` just construct it with different args. `ProjectDetailsPage(projectId, edit)` fetches the project via `ProjectDetailsBloc` then renders `AddProjectPage(readOnly: !edit || role != RoleType.admin)` — **only admin can edit**, everyone else (including the assigned manager) gets the read-only view. `ProjectCreateBloc` drives both `ProjectCreateSubmitted` and `ProjectUpdated`, plus document add/remove (documents are **name+link pairs**, not file uploads — a value that parses as an http(s) URI opens externally on tap, copies on long-press). Manager is a single-select dropdown; employees/testers are multi-select via `Routes.taskMultiSelect` (same `TaskMultiSelectPage` reused app-wide). The "Faol" UI toggle is inverted from the API's `is_hidden` field (`_active = !project.isHidden`) — don't wire it to `isHidden` directly.
...
- `GET /meeting-attendance/?meeting={id}` + `PATCH /meeting-attendance/{id}/` — per-user attendance rows; the **absence reason is submitted here** (`{absence_reason}`), **not** on the meeting. A meeting notification only carries `meeting_id`, so `MeetingReasonBloc` first resolves the attendance row id via the filtered list endpoint, then PATCHes it. Don't shortcut this to `PATCH /meetings/{id}/` — that was the original wrong contract (real schema confirmed otherwise).

`MeetingCard`'s attendance badge: `ic_check` is a **composite** asset (green square + white check) — render it with **no `colorFilter`** or it recolors solid. The "not attended" badge is drawn from theme tokens (`errorSub` square + `iconWhite` minus), same as the reason-page submit check (`_CheckPainter`). Notification → reason deep-link lives in `notification_page.dart` `_onNotificationTap` (`type == 'meeting' || action == 'open_meeting'`, needs `extra_data.meeting_id > 0`).

### Profile settings (logout / theme / security / role switch)
All under `features/profile/presentation/`:
- **Logout** (`logout_confirm_dialog.dart`) — centered confirm dialog, fires `SessionLogoutRequested`; router guard handles the redirect (never navigate imperatively).
- **Theme switcher** (`theme_switch_sheet.dart`) — bottom sheet, persists to `StorageKeys.themeMode` **and** calls `AppOptions.update`. Startup reads it in `app/app.dart` via `StorageService` (was hardcoded `ThemeMode.system`). Adding a new persisted app-option follows this pair: write key on change + read key at startup.
- **Security page** (`security_page.dart`) — "Parol o'zgartirish" row opens `change_password_dialog.dart`; "Avtomatik qulflash: 3 daqiqa" is info-only (mirrors `SessionBloc.pinLockTimeout`).
- **Change password** — request body is `{old_password, new_password, confirm_new_password}` (all three required; endpoint `/users/me/change-password/`). On success `ChangePasswordBloc` **rewrites `StorageKeys.pinLength`** to the new password length — PIN **is** the password, so a stale `pinLength` leaves the PIN screen showing the wrong slot count. Any flow that changes password length must update `pinLength`.
- **Role switch** (`role_switch_dialog.dart`) — reuses `RoleSelectBloc` (`PATCH /users/me/`); on backend success the caller updates `SessionBloc` + reloads `ProfileBloc` and shows a success toast.
...
### Reports (users / projects / expenses) + shared filter kit
`features/reports/` hosts three parallel report lists, all three-layer: **user** (`GET /reports/users/`), **project** (`GET /reports/projects/`), **expense** (`GET /reports/expenses/`). Entry point is the "Hisobotlar" tab (`home/.../reports_page.dart`) — `AppNavTile`s push `Routes.userReports` / `projectReports` / `expenseReports` (Wages/Tasks tiles are stubs; the "Foydalanuvchilar" and "Moliya" bottom-nav tabs are still bare `TabPlaceholder`s, not yet built). Each report = list page + filter page + card + list bloc + filter bloc. All three list endpoints are **paginated** (`{count, next, results}`); list blocs track `page`/`hasReachedMax`/`isLoadingMore` and infinite-scroll at `maxScrollExtent - 300`. Filter is a single immutable value object (e.g. `ExpenseReportFilter`) carried whole: the header's search field emits `...SearchChanged` (via `copyWithSearch`, so filters survive), the filter page returns the full object via `pop`, list bloc replaces state filter on `...FilterChanged`. The data source maps that object → query params in one `_…FilterParams` map (only non-empty keys), sets on `hasActiveFilters` drive the header filter-button dot.

**Shared filter widgets** live in `core/widgets/app_filter_components.dart` — reuse these, don't rebuild: `AppFilterHeader` (back + centered title), `AppFilterFieldLabel`, `appFilterFieldDecoration(colors)` (the 44h boxed-field look), `AppFilterFieldBox` (label + value/placeholder + clear-×/chevron), `AppFilterPickerBox` (date/time trigger), `AppFilterDropdownBox`/`AppFilterDropdownItem` (the inline dropdown surface), `AppFilterActionBar` (Tozalash + Shakllantirish, `applyIcon` overridable). Two selection patterns coexist and are chosen per field: **inline single-select** = `OverlayPortal` + a `LayerLink` per field + `AppFilterDropdownBox` (see `user_reports_filter_page.dart` / expense filter's `type`/`status`/`category`); **full-page multi-select** = push `Routes.taskMultiSelect` (`TaskMultiSelectPage`) with `MultiSelectItem`s (carries `avatarUrl`/`subtitle` → `TuiAvatar` + position). For people/project pickers always build `MultiSelectItem` from `UserShort` (`id/username/position/avatar`) so avatars render — don't hand-roll a bare `{id,title}` list. Boxed number inputs must wrap the `TextField` in `Center()` (raw `SizedBox(height: 44)` top-aligns the text); the expense filter's amount fields use a live `_ThousandsInputFormatter` (`10000` → `10 000`) and strip spaces before `num.tryParse`.

**Expense type vs category — confirmed contract (they are different axes, not a hierarchy):** the report's `type` (Xarajat turi) is a fixed string enum `withdrawal`/`company`/`other` → filter param `type`; `expense_category` (Xarajat toifasi) is an **FK id** → filter param `expense_category`, and its options come from `GET /expense-category/` (**non-paginated flat array** of `{id, title}` — send no `page_size`). `GET /expense-category/{id}/` returns the same flat object (no nested sub-categories) — there is **no** turi→toifa parent/child, so don't wire the category dropdown off the retrieve endpoint. Filter-option blocs load their pick-lists in parallel: `ReportsFilterBloc` (positions + users + regions), `ProjectReportsFilterBloc` (users), `ExpenseReportsFilterBloc` (projects + categories via options use-case **plus** `GetUsersUseCase` for user/accountant avatars).
...
- Dark-mode token values have drifted from Figma before (`backgroundElevation1Alt` dark was `#222323`, a leftover/guessed value — Figma's real dark variable is `#1C2128`). When a dark screen looks visibly off, pull the node's variable defs from Figma and diff against the `dark` constructor block in `app_colors.dart` before adding a new token — it may just be an existing one with a wrong hex.
